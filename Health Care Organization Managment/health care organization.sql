...
ALTER TABLE Patient_release
ADD prescription VARCHAR(50);

ALTER TABLE Patient_release 
DROP COLUMN prescription;

ALTER TABLE Patient_release 
ADD bill_amount number(30);
...
select P_id,name as released_patient from Patient where P_id in 
(select Patient_release_id from Patient_release where  Patient.P_id=Patient_release.Patient_release_id and bill_paid='Yes');

select name as Patient_under_treatment from Patient where P_id in
(select Patient_id  from treatment where Patient.P_id=Treatment.Patient_id );
...
SELECT b1.bill_amount,b2.bill_amount 
   FROM Patient_release b1 JOIN Patient_release b2 ON b1.bill_amount=b2.bill_amount;
...
SET  SERVEROUTPUT ON
DECLARE
  t_date Treatment.treatment_date%type;
  p_name  Patient.name%type := 'Rahim';
BEGIN
  SELECT treatment_date INTO t_date
  FROM Treatment, Patient
  WHERE Treatment.Patient_id = Patient.P_id AND
  name = p_name;  
  DBMS_OUTPUT.PUT_LINE('Patient : ' || p_name || ' treatmented on '|| t_date);
END;
/
